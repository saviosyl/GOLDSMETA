...
const OUT = path.resolve(__dirname, "../../docs/v5-4-3-from-v541");
const BASELINE_DIST = "/tmp/gm-c7e427d/web/dist";
  ...
  for (const shot of shots) {
    ...
    if (shot.prep === "score") {
      ...
      if (await toggle.count()) {
        ...
        await page.getByTestId("score-row-news").scrollIntoViewIfNeeded().catch(() => {});
      }
    }
    if (shot.prep === "map") {
...
const baselineServer = await serve(BASELINE_DIST, 4191);
const repairServer = await serve(REPAIR_DIST, 4192);
await capture("baseline-c7e427d", "http://127.0.0.1:4191");
await capture("repaired", "http://127.0.0.1:4192");
baselineServer.close();
repairServer.close();
console.log("captured c7e427d baseline + repaired screenshots to", OUT);
