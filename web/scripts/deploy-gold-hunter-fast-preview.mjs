#!/usr/bin/env node
/**
 * Isolated GOLD_HUNTER FAST live-shadow preview deploy (Cloudflare Pages).
 * Does NOT deploy production custom domain. noindex/nofollow via env flag.
 * Never logs secret values.
 *
 * Required env:
 *   CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID
 *   VITE_FIREBASE_* (same as production web config — auth shell only)
 *   VITE_API_BASE_URL (production API ok — FAST live data comes from health URL)
 *   VITE_GOLD_HUNTER_FAST_HEALTH_URL
 */
...
const healthUrl = (process.env.VITE_GOLD_HUNTER_FAST_HEALTH_URL || "").trim();
if (!healthUrl) {
  console.error("FAIL: VITE_GOLD_HUNTER_FAST_HEALTH_URL required");
  process.exit(1);
}
...
process.env.VITE_GOLD_HUNTER_FAST_PREVIEW = "true";

const projectName = await resolvePagesProject();
console.log("== build FAST live-shadow preview ==");
...
const message =
  ...
  "gold-hunter-fast live-shadow preview (noindex)";
...
const deployArgs = [
  ...
  projectName,
  "--branch",
  "preview-gold-hunter-fast-shadow",
  "--commit-dirty=true",
  "--commit-message",
  message
];
...
const match =
  ...
  out.match(/https:\/\/preview-gold-hunter-fast-shadow\.[a-z0-9-]+\.pages\.dev/i);
console.log("PREVIEW_URL=" + (match?.[0] ?? "(see wrangler output)"));
console.log("Health host only:", new URL(healthUrl).host);
