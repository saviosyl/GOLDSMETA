import { Activity, Bot, CheckCircle2, CircleDollarSign, Cpu, Radio, ShieldCheck, Target, XCircle } from "lucide-react";
...
export function GoldHunterOverviewV2Page() {
  ...
  return (
    <div className="gh26-page" data-testid="gh26-overview">
      <section className="gh26-hero">
        <div className="gh26-hero__top">
          <div>
            ...
            <h1>Gold Hunter</h1>
            <p className="gh26-runtime-note">Trading identity comes from the live Gold Hunter runtime. Web build identity is shown separately.</p>
          </div>
      ...
      <section className="gh26-identity gh26-identity--runtime" aria-label="Gold Hunter runtime identity">
        <article><Bot aria-hidden /><span>Trading Brain</span><strong>{brain}</strong><small>{status.strategyVersions.brainVersion}</small></article>
        <article><Activity aria-hidden /><span>Brain Revision</span><strong>{revision}</strong></article>
        <article><Target aria-hidden /><span>Variant</span><strong>{strategy}</strong></article>
        <article><Cpu aria-hidden /><span>Software Revision</span><strong>{softwareRevision || "—"}</strong></article>
        <article><Radio aria-hidden /><span>API Revision</span><strong>{status.runtimeSha ?? "Not exposed"}</strong></article>
        <article><Radio aria-hidden /><span>Quote Worker</span><strong>{status.strategyVersions.workerRevision ?? "Not exposed"}</strong><small>{status.strategyVersions.telemetrySource ?? "Telemetry source unavailable"}</small></article>
        <article className="gh26-web-build"><Cpu aria-hidden /><span>Web Build</span><strong>{goldHunterBuildShort()}</strong><small>Presentation build only</small></article>
      </section>
