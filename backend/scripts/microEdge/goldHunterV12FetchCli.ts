/**
 * GOLD_HUNTER V1.2 — fetch REAL Pepperstone DEMO history for the
 * NEW older development window ending BEFORE 2026-07-09 (prior to V1.1).
 *
...
function initFirebase(): void {
  ...
  initializeApp({ projectId: process.env.GCLOUD_PROJECT || "local-dev" });
}
...
function assertGoldPrices(ticks: RawTick[]): void {
  ...
  if (sample.length && bad / sample.length > 0.01) {
    throw Object.assign(
      new Error(`V12_TICK_PRICE_SANITY_FAILED: ${bad}/${sample.length}`),
...
async function main(): Promise<void> {
  ...
  if (creds.environment !== "DEMO") throw new Error("V12_DEMO_ONLY");

  const toMs = V12_DEV_WINDOW_END_MS; // exclusive of V1.1 development start
  const fromMs = toMs - V12_DEV_WINDOW_DAYS * 24 * 60 * 60 * 1000;
  const outDir =
  ...
  console.log(
    JSON.stringify({
      event: "gh_v12_fetch_start",
      fromUtc: new Date(fromMs).toISOString(),
      toUtc: new Date(toMs).toISOString(),
      days: V12_DEV_WINDOW_DAYS,
      note: "ends before V1.1 development window",
      reuse
    })
  );
  ...
  if (
    ...
    console.log(JSON.stringify({ event: "gh_v12_reuse_local", meta }));
    return;
  }
  ...
  try {
    ...
    console.log(JSON.stringify({ event: "gh_v12_fetch_bars" }));
    ...
    const meta = {
      datasetRole: "GH_REAL_56D_PRE_V11",
      fromUtc: new Date(fromMs).toISOString(),
      toUtc: new Date(toMs).toISOString(),
      M1: m1.length,
      M5: m5.length,
      ...
      v11DevExcludedFrom: new Date(V12_DEV_WINDOW_END_MS + 1).toISOString()
    };
    writeFileSync(metaPath, JSON.stringify(meta, null, 2));
    console.log(JSON.stringify({ event: "gh_v12_fetch_done", ...meta }));
    console.log(JSON.stringify({ event: "gh_v12_fetch_persisted", outDir }));
  } finally {
