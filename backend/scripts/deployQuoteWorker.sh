...
gcloud run deploy "$SERVICE" \
  ...
  --port=8080 \
  --service-account="$SA" \
  --set-env-vars="GCLOUD_PROJECT=${PROJECT},CTRADER_CONNECTOR_ENABLED=true,CTRADER_DEMO_READ_ENABLED=true,CTRADER_LIVE_ENABLED=false,CTRADER_QUOTE_REQUIRE_LIVE=true,CTRADER_QUOTE_PREFER_STORE=true" \
  --update-secrets="CTRADER_CLIENT_ID=CTRADER_CLIENT_ID:latest,CTRADER_CLIENT_SECRET=CTRADER_CLIENT_SECRET:latest,CTRADER_REDIRECT_URI=CTRADER_REDIRECT_URI:latest,CTRADER_ENVIRONMENT=CTRADER_ENVIRONMENT:latest,CTRADER_TOKEN_ENCRYPTION_KEY=CTRADER_TOKEN_ENCRYPTION_KEY:latest,GOLDMETA_PINNED_OWNER_UID=GOLDMETA_PINNED_OWNER_UID:latest"
